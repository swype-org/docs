...
- [Security & Content Integrity](https://docs.blink.cash/integration/security) — required reading for Phase 5a (web CSP)
...
10. **Lock the iframe origin (recommend for production).** Add — or, if you manage the app's response headers/middleware, propose adding — a Content-Security-Policy on the top-level page that restricts the deposit iframe to Blink: `frame-src https://pay.blink.cash` (add `https://pay-sandbox.blink.cash` for sandbox) with `script-src 'self'`. `connect-src` only needs the app's own signer endpoint — Blink's backend/Privy traffic runs inside the cross-origin iframe, not the host page. See [Security & Content Integrity](https://docs.blink.cash/integration/security).
...
- (Web) Lock the deposit iframe origin with a `frame-src https://pay.blink.cash` Content-Security-Policy on the app's pages. See [Security & Content Integrity](https://docs.blink.cash/integration/security).
- Verify the SDK supply chain with `npm audit signatures` (`@swype-org/deposit` is published with npm provenance); optionally diff the live UI against `https://pay.blink.cash/manifest.json`.
