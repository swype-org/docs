---
title: Integrate the Transfer SDK
description: Install and configure the Transfer SDK to trigger payment flows from your frontend.
---

The Transfer SDK (`@swype-org/transfer`) opens Blink's hosted payment flow in a modal iframe overlay. On mobile viewports (480px or narrower), the iframe renders full-screen for an app-like experience. The SDK handles the signer call, iframe lifecycle, passkey/WebAuthn delegation, and completion detection via `postMessage`.
...
```bash npm
npm install @swype-org/transfer
```

```bash yarn
yarn add @swype-org/transfer
```

```bash pnpm
pnpm add @swype-org/transfer
```
...
```typescript
import { Transfer, TransferError, getDisplayMessage } from '@swype-org/transfer';
...
document.getElementById('transfer-btn')!.addEventListener('click', async () => {
  try {
    const { transfer: result } = await transfer.requestDeposit({
      amount: 50,
      chainId: 8453,
...
```tsx
import { useBlinkTransfer } from '@swype-org/transfer/react';

function TransferButton() {
  const { status, result, error, displayMessage, requestDeposit } = useBlinkTransfer({
    signer: '/api/sign-payment',
  });
...
The hook returns reactive `status`, `result`, `error`, `displayMessage`, and `isActive` values, plus `requestDeposit`, `focus`, and `close` actions. It manages the `Transfer` lifecycle and cleans up on unmount.
...
```typescript
import type { SignerFunction } from '@swype-org/transfer';
...
```typescript
transfer.on('status-change', (status) => console.log('Status:', status));

transfer.status;    // current status
transfer.result;    // last DepositResult (when completed)
transfer.error;     // last TransferError (when error)
transfer.isActive;  // true during signer-loading or iframe-active
```

## Error handling

See [Error Codes](/sdk-reference/errors) for the full reference. Every error is a `TransferError` with a machine-readable `code`:
...
```typescript
// Close the transfer iframe without waiting for completion
transfer.close();
