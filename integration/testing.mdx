...
| `SAMPLE_APP_LINK_TTL_SECONDS` | Payment link TTL in seconds | `600` |
...
<Steps>
  ...
  <Step title="Launch the checkout flow">
...
This logs signer calls, iframe state transitions, postMessage events, and errors to `console.debug` with the `[SwypeCheckout]` prefix.
