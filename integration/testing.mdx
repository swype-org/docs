...
- Transfer SDK initialization and `requestDeposit()` call (see `src/PaymentPage.tsx`)
...
<Steps>
  ...
  <Step title="Launch the transfer flow">
...
- Point the Transfer SDK's `signer` config at your deployed signer URL.
...
This logs signer calls, iframe state transitions, postMessage events, and errors to `console.debug` with the `[BlinkTransfer]` prefix.
