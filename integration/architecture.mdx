...
sequenceDiagram
    ...
    participant SDK as Transfer SDK
...
|-----------|---------|----------------|
| **Merchant App** | Browser | Your frontend. Calls `requestDeposit()` on the Transfer SDK when the user wants to pay. |
...
| Component | Runs on | Responsibility |
|-----------|---------|----------------|
| **Transfer SDK** | Browser (in your app) | Orchestrates the signer call, opens the iframe, and listens for the completion `postMessage`. Published as `@swype-org/transfer`. |
...
1. **Private key stays on your server.** The Transfer SDK never sees your private key. It only receives the signed payload from your signer endpoint.
