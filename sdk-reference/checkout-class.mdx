...
description: API reference for the Checkout class — the core SDK entry point.
---
...
| `debug` | `boolean` | `false` | Log lifecycle events to `console.debug` with the `[SwypeCheckout]` prefix. |
...
Opens the hosted payment flow for the given deposit. The returned Promise resolves when the user completes the payment and rejects with a `CheckoutError` on failure.
