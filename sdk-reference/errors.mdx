...
description: Error codes, CheckoutError class, and user-friendly error messages.
---

Every error thrown by the Checkout SDK is a `CheckoutError` instance with a machine-readable `code` property. This lets you branch on failure mode without parsing message strings.
...
|------|---------|---------------------|
| `CHECKOUT_DISMISSED` | User dismissed the checkout (tapped backdrop, pressed Escape, or closed the flow). | "The checkout was dismissed before the transfer completed." |
...
| `SIGNER_RESPONSE_INVALID` | Signer response missing required fields (`merchantId`, `payload`, `signature`, `expiresAt`). | "The payment server returned an unexpected response." |
...
Returns a user-friendly display string for a `CheckoutError`. You can show this directly in your UI.

```typescript
function getDisplayMessage(error: CheckoutError): string
```
...
The `useSwypeCheckout` hook provides `displayMessage` as a convenience:

```tsx
const { error, displayMessage } = useSwypeCheckout({ signer: '/api/sign-payment' });
